...
def test_real_generation_synthetic_spectrum_explains_rank_change_and_sensitivity():
    ...
    result = module.evaluate_scenario(*scenario).set_index("annotation")
    assert result.loc["SM(d18:1/16:0)","legacy_rank"] == 1
    assert result.loc["SM(d16:1/18:0)","multi_evidence_rank"] == 1
    assert result.loc["SM(d18:1/16:0)","multi_evidence_rank"] == 2
